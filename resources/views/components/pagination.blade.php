...
<div class="mt-6 flex flex-wrap justify-center items-center gap-4 px-4">
    @if($currentPage > 1)
        <a href="{{ route($route, ['page' => $currentPage - 1]) }}"
           class="flex items-center px-3 sm:px-4 py-2 bg-dark-light rounded-lg text-white hover:text-primary-500 transition">
    ...
    <div class="flex items-center">
        <span class="hidden sm:inline text-white text-sm sm:text-base">Page</span>
        <span class="mx-2 px-3 py-1 bg-dark rounded-lg text-primary-500 font-medium text-sm sm:text-base">
            {{ $currentPage }}
        </span>
        <span class="text-white text-sm sm:text-base">sur {{ $totalPages }}</span>
    </div>
